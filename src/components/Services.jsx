     ...
     return (
          ...
          <div className={`mx-[1rem] px-[1rem] mb-[5rem] py-[2rem] rounded-lg shadow-lg shadow-gray-400 bg-gray-200 flex flex-col gap-[1rem]`}>
               {finesUnpaidLoading ? (
                    ...
                    <div className={`text-lg font-semibold min-h-[5rem] flex justify-center items-center`}>Nta muntu utarishyura</div>
               ) : filteredFInes.length <= 0 ? (
                    <div className={`text-wrap break-words`}>Nta gihano kitarishyurwa &quot; { searchQuery } &quot;</div>
                    ...
                    return (
                         <div className={`bg-white p-[1rem] flex flex-col gap-[.5rem] rounded-lg shadow-md shadow-gray-400 last-of-type:mb-[1rem] border-l-4 border-[#301B84]`} key={index}>
                              <p className={`font-semibold text-xl`}><span>{index + 1}</span>.&nbsp;&nbsp;<span>{fine.userId?.name || "Nta zina"}</span></p>
                              <div className={`flex justify-between px-[1rem]`}>
                                   <p className={`text-gray-600 text-sm`}>{fine.date}</p>
                                   <p className={`text-sm ${fine.fineStatus ? "text-green-600" : "text-red-600"} font-semibold`}>{fine.fineStatus ? "Yarishyuye" : "Ntarishyura"}</p>
                              </div>
