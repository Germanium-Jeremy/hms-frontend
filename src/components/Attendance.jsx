...
const Attendance = () => {
     ...
     const handleSubmit = async (e) => {
          ...
          try {
               ...
               if (error.response.data.error.toLowerCase().includes("duplicate key")) {
                    toast.warn("Wari wamaze kwemeza attandance");
               } else if (error.response.data.message.toLowerCase().includes("invalid date format")) {
                    toast.warn("Suzuma itariki wongere wemeze");
               } else {
                    toast.warn("Kari akabazo");
               }
          }
     };
     ...
     return (
          <>
          <SearchBar item={"umumambre"} itemFunction={handleSearchChange} itemValue={searchQuery} />
          <h2 className={`text-center text-xl font-bold my-[1rem]`}>Attandance</h2>
          
          <div className={`flex flex-row justify-between items-center px-[1rem]`}>
               <Link to={'../attend'}>Reba Attandace y'uyu munsi</Link>
               <Link to={'/'}>Reba attandance zose</Link>
          </div>
          ...
          <div className={`text-white bg-[#301B84] mx-[1rem] rounded-lg py-[.5rem] px-[2rem] flex justify-between`}>
               ...
               <span className={`flex gap-[.5rem] items-center`}>
                    <input type="date" className={`text-black px-[.5rem]`} value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} />  Itariki 
               </span>
          </div>
          <form onSubmit={handleSubmit} className={`mx-[1rem] px-[1rem] mb-[5rem] py-[2rem] rounded-lg shadow-lg shadow-gray-400 bg-gray-200 flex flex-col gap-[1rem]`}>
               {allUsersLoading ? (
                    ...
                    <div className={`break-words p-[1rem] min-h-[5rem] rounded bg-gray-500`}>Nta muntu ufite &quot; { searchQuery } &quot;</div>
               ...
               {submitLoading ? (
                    <button className={`px-[2rem] py-[.7rem] rounded-lg bg-gray-500 text-white mx-[3rem] mt-[1rem]`}>Tegereza...</button>
                    ) : (
                    <button className={`px-[2rem] py-[.7rem] rounded-lg bg-[#301B84] text-white mx-[3rem] mt-[1rem]`}>Emeza Impinduka</button>
               )}
          </form>
          </>
     )
