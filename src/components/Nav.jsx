...
const Nav = ({isMenu, setIsMenu}) => {
     ...
     return (
          <>
          <div className={`fixed top-0 bottom-0 right-0 z-[2] flex transition ${isMenu ? 'left-0 right-0' : 'left-[-100%] right-[100%]'}`}>
               <div className={`flex flex-col gap-[2rem] w-full h-full bg-white px-[2rem] py-[3rem] justify-center`}>
                    {menuLinks.map((link, index) => {
                         return (
                              <Link to={link.ancor} key={index} className={`text-lg indent-[3rem] font-semibold ${activeLink == link.ancor ? "border-b-4 border-[#301B84]" : ''}`} onClick={() => {
